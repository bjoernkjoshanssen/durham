...
def point_achieved
  (l : List (ℤ×ℤ)) (a b : Fin l.length) (phobic : Vector Bool l.length) : Prop
:= by {
  exact phobic.get a ∧ phobic.get b ∧ a.succ < b ∧ nearby' (l.get a) (l.get b)
}
...
theorem first_point' : point_achieved' f₁ 0 3 phobe
:= by decide -- easier to "decide" when everything is infinite
-- because there are no proofs of finiteness floating around.
...
theorem first_point (b₀ b₁ : Bool) : point_achieved myfold 0 3 ⟨[true,b₀,b₁,true],fpl⟩
  := by {
    -- unfold point_achieved;unfold myfold;unfold nearby';unfold down;
    -- unfold up;unfold left;unfold right;simp;

    constructor;simp;rfl;constructor;rfl
