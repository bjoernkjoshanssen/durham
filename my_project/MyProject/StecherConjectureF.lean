...
(j:Fin l.succ.succ) :
  ...
  → { i_1 // pt_locF rect (fun i => reflect (fold i)) i_1 j ph = true }
    := (λ x ↦ ⟨x.1,by
      unfold pt_locF;simp
      let Q := x.2;unfold pt_locF at Q;simp at Q
...
(fold: Fin l.succ.succ → ℤ × ℤ)
       : ∀ i,
        ...
        pts_at'F rect i ph (fun i => reflect (fold i)) := by
          ...
          simp; apply Fintype.card_of_bijective (reflect_pts₁_bijective _ _ _)
...
(fold: Fin l.succ.succ → ℤ × ℤ)
       : ∀ i,
        ...
        pts_at'F rect i ph (fun i => rotate (fold i)) := by
          intro i;unfold pts_at'F;repeat rw [← Fintype.card_coe]
          simp; apply Fintype.card_of_bijective (rotate_pts₁_bijective _ _ _)
...
(fold: Fin l.succ.succ → ℤ × ℤ)
       : ∀ i,
        ...
        pts_at'F rect i ph (fun i => rotate (rotate (rotate (fold i)))) := by
          intro i;unfold pts_at'F;repeat rw [← Fintype.card_coe];simp
...
: ∃ fold', ∃ _ : isPathWitness rect fold',
 ...
 := by
  ...
  . exists fold;exists f;simp;let Q := (f 0).2;simp at Q
    rw [h] at Q;exact Q
  cases h -- 1
  . exists (λ i ↦ rotate (fold i))
    ...
    . let Q := (f 0).2;simp at Q;simp;rw [Q];(repeat rw [rotate_basic]);congr
  cases h_1 -- 2, 3
  . exists (λ i ↦ rotate (rotate (fold i)))
    ...
    . let Q := (f 0).2;simp at Q;congr;simp;rw [Q];repeat rw [rotate_basic]
      congr
  . exists (λ i ↦ rotate (rotate (rotate (fold i))))
    ...
    constructor
    . unfold pts_tot'F;rw [funext (rotate₃_pts_at'F _ _)]
    . let Q := (f 0).2;simp at Q;simp;rw [Q];repeat rw [rotate_basic]
      rw [h]
...
: (f (Fin.castSucc k)).1 = 0 → (f (Fin.succ k)).1 = 1 → ¬ Function.Injective fold
 := by
  ...
  have hcanc: ∀ x, rect 1 (rect 0 x) = x := by
    intro x;show x + (1,0) + (-1,0) = x;apply Prod.ext;simp;simp
  intro h₀ h₁ hc
...
: ∃ fold', Function.Injective fold'
  ...
  . exists fold''
    ...
    . unfold isPathWitness at f'' -- this is awkward because the "wrong" thing was proved before
      ...
      exact R
    . intro h;exfalso;contrapose then_if;simp;tauto
