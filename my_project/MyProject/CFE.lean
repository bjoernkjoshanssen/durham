...
theorem eexample (r:ℝ) (h: 0 < r) : ∃ n : ℕ, 1/(n+1) < r := exists_nat_one_div_lt h

theorem dexample {r:ℝ} (h: ∀ n : ℕ, r ≤ 1/(n+1)) : r ≤ 0 := by {
  by_contra hh
  have : 0 < r := by exact not_le.mp hh
  have : ∃ n : ℕ, 1/(n+1) < r := eexample r this
  cases this
  have hr : r ≤ 1/(w+1) := h w
...
example (a b : ℕ) (h : a ≤ b) (hb : 0<b) (ha : 0<a) :
  (1:ℝ)/b ≤ 1/a := by {
...
-- theorem Cool (r:ℚ) (h: ∀ n : ℕ, r ≤ 1/2^(n)): ∀ n : ℕ, r < 1/(n+1) := by {

--   intro n
--   have hr : r ≤ 1/2^(n+1) := h (n+1)
--   have : n+1 ≤ 2^(n+1) := id_pow (n+1)

--   have hn : 0 < (n:ℤ)+1 := sorry
-- --   -- have : n < 2^(n+1) := sorry
--   have ht: 0 < (2^(n+1):ℤ) := sorry
--   have hnn : 1/2^(n+1) ≤ Rat.div 1 (n+1) := sorry
  --Nat.div_le_div_left this hn
--   -- problem is that Nat.div_le_div refers to DIV not fractions!
  -- have : Rat.div 1 2^(n+1) < Rat.div 1 (n:ℚ) ↔ 1*n < 1*2^(n+1) :=
  -- Rat.div_lt_div_iff_mul_lt_mul ht hn

  -- have : Rat.div 1 2^(n+1) < Rat.div 1 n := sorry
