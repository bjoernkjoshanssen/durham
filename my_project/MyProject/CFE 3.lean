...
theorem emptyNotRandom : (∅ : Set ℕ) ∈ U_half 0 := by {
  exists 1
  constructor
  exact Nat.zero_le 1
  have : average (∅: Set ℕ) (N 1) = 0 := by {
    have : N 1 = 4 := (calc
    N 1 = 2^(3*1-1) := rfl
    _   = 2^(3-1)   := by ring
    _   = 2^2       := by ring
    _   = 4         := rfl)
    rw [this]
    have h4: Nat.sum (List.ofFn (λ k : Fin 4 ↦ natOfProp ( (∅:Set ℕ) k.1))) = 0 := by {
      have seriously : ∀ k : Fin 4, (propDecidable (k.1 ∈ (∅: Set ℕ))) = (Set.decidableEmptyset k.1) :=
        λ k ↦ by {simp}
      have : ∀ k : Fin 4, k.1 ∈ (∅:Set ℕ) ↔  (∅:Set ℕ) k.1 := λ k ↦ Set.mem_def
      have : ∀ k : Fin 4, natOfProp (k.1 ∈ (∅:Set ℕ)) = natOfProp ((∅:Set ℕ) k.1) := by {
        intro k
        rw [← seriously k]
        exact eq_of_iff (this k)
      } --sorry --λ k ↦ eq_of_iff (this k)
      have : (λ k : Fin 4 ↦ natOfProp (k.1 ∈ (∅:Set ℕ)))
           = (λ k : Fin 4 ↦ natOfProp ((∅:Set ℕ) k.1)) := funext this
      rw [← this]
      rfl
    }
    have : summing (∅: Set ℕ) 4 = 0 := by {unfold summing;rw[h4]}
    unfold average
    rw [this]
    exact mul_zero _
  }
