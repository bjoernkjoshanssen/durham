...
def point_achieved_with₁
  (l : ℕ → (ℤ×ℤ)) (a b : ℕ) (phobic : ℕ → Bool) : Bool
:= by {
  exact phobic a && phobic b && a.succ < b && nearby_with₀ move_fin (l a) (l b)
}
...
(k : ℕ) (ph : ℕ → Bool)
  (f : ℕ → ℤ×ℤ) :=
  List.sum (List.map F (List.ofFn (λ i : Fin k ↦   point_achieved_with f i k ph)))
...
(u ++ v).get ⟨u.length + m,by {rw [List.length_append];simp}⟩
     = v.get m := by {
  have h : ¬(m + u.length) < u.length := by {intro hc; exact Nat.not_succ_le_zero m.1 (Nat.add_lt_add_iff_right.mp (Nat.lt_add_left 0 hc))}
...
(h : Function.Injective (λ i ↦ (u ++ v).get i)) :
    ...
    have : List.get (u++v) ⟨u.length + m,by {rw [List.length_append];simp}⟩
         = List.get (u++v) ⟨u.length + n,by {rw [List.length_append];simp}⟩ := by {
...
theorem injective_prefix {α:Type} (u v : List α)
  ...
  intro m n hmn
  have hm: u.get m = (u++v).get ⟨m.1,by {rw [List.length_append];exact Nat.lt_add_right (List.length v) m.2}⟩
    := (List.get_append_left _ _ _).symm

  have hn: u.get n = (u++v).get ⟨n.1,by {rw [List.length_append];exact Nat.lt_add_right (List.length v) n.2}⟩
    := (List.get_append_left _ _ _).symm
...
-- January 27, 5:30pm
  ...
  by {
    ...
    intro x
    intro a
    exists (go a ((walk' go x).1.head (by {
      intro hc
      let G := congr_arg List.length hc
      simp at G
      let  H := moves_len' go x
      rw [H] at G
      contrapose G
      simp
    })))
...
by {
  unfold walk
  have : x.reverse <:+ z.reverse := by exact List.reverse_suffix.mpr h
  let G:= moves_extend' go this
  assumption
}
...
by {
  have : x.reverse <+: z.reverse := by exact List.reverse_prefix.mpr h
  let G := moves_extend go this
