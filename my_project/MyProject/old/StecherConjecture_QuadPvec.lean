...
def pt_earned {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
...
def pt_property {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
 {l : ℕ} (fold : Vector α l) (i j : Fin l) (phobic : Vector Bool l) (a:β)
:=
phobic.get i && phobic.get j && i.1.succ < j.1 && fold.get j = go a (fold.get i)


theorem pt_earned_iff_ex
 {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
 {l : ℕ} (fold : Vector α l.succ) (i j : Fin l.succ) (phobic : Vector Bool l.succ)
 : pt_earned go fold i j phobic
 ↔
∃ a, pt_property go fold i j phobic a := by
  unfold pt_property;
  constructor;intro h;unfold pt_earned at h;unfold nearby at h;simp at h
  rcases h.2 with ⟨a,ha⟩;exists a;simp;tauto;intro h;unfold pt_earned
  rcases h with ⟨a,ha⟩;unfold nearby;simp;simp at ha;constructor;tauto;exists a;tauto

-- Now we can use pt_earned_iff_ex with our ∃ a, ∃ i theorem.

def pts_at {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l) : ℕ :=
  Finset.card (
    Finset.filter (λ i : Fin k ↦ (pt_earned go fold ⟨i.1,finfin i⟩ k ph))
    Finset.univ -- should really make this i : Fin l.
  )

def pts_at' {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l) : ℕ :=
  Finset.card (
    Finset.filter (λ i : Fin l ↦ (pt_earned go fold i k ph))
    Finset.univ -- should really make this i : Fin l.
  )
/-
prove that pts_at  = pts_at' actually, since:
  pts_at' is more convenient type-theoretically, but
  pts_at is more amenable to proving a certain bound.
-/
def change_type  {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l):
Finset.filter (λ i : Fin l ↦ (pt_earned go fold i k ph)) Finset.univ
→
Finset.filter (λ i : Fin k ↦ (pt_earned go fold ⟨i.1,finfin i⟩ k ph)) Finset.univ
  := by
    intro ip; let Q := ip.2; rw [Finset.mem_filter] at Q -- Finset.mem_filter was key here
    unfold pt_earned at Q;
    have : ip.1.1.succ < k := by
      simp at Q;tauto
    exact ⟨⟨ip.1.1,Nat.lt_of_succ_lt this⟩,by rw [Finset.mem_filter];simp;tauto⟩

theorem change_type_bijection
  {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l)
:
Function.Bijective (change_type go k ph fold) := by
  constructor
...
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l):
Fintype.card (Finset.filter (λ i : Fin l ↦ (pt_earned go fold i k ph)) Finset.univ)
=
Fintype.card (Finset.filter (λ i : Fin k ↦ (pt_earned go fold ⟨i.1,finfin i⟩ k ph)) Finset.univ)
:= by
  apply Fintype.card_of_bijective
  exact change_type_bijection go k ph fold
...
/- can't really assert that pts_at and pts_at' are "the same" though as l:ℕ
has to be inferred -/
...
theorem pt_earned_of_embeds
 {α:Type} [DecidableEq α] {b₀ b₁ : ℕ}
{go₀ : Fin b₀ → α → α} {go₁ : Fin b₁ → α → α} (h_embed : embeds_in go₀ go₁)
 {l:ℕ}
  (fold : Vector α l) (a b : Fin l) (phobic : Vector Bool l)
  (htri: pt_earned go₀ fold a b phobic) :
         pt_earned go₁ fold a b phobic := by
  unfold pt_earned
  unfold pt_earned at htri
...
pts_at go₁ k ph fold := by
  ...
  simp
  simp at ha
  exact pt_earned_of_embeds h_embed fold ⟨a,finfin a⟩ k ph ha
...
pts_at' go₁ k ph fold := by
  ...
  simp
  simp at ha
  exact pt_earned_of_embeds h_embed fold a k ph ha
...
{l:ℕ} (ph : Vector Bool l) (fold : Vector α l) (i:Fin l):
pts_at' go i ph fold ≤ i := by
  rw [← pts_at_eq_pts_at'] -- Hooray!
  rw [← Finset.card_fin i.1]
  apply Finset.card_le_card
  apply Finset.filter_subset


lemma pts_earned_bound_aux {α:Type} [DecidableEq α]
{β : Type} [Fintype β]
(go : β → α → α)
...
/- For [true] the bound is 0 (which is sharp);
  ...
  calc
  _ ≤ ∑ j : Fin l.succ, j.1 := pts_earned_bound_aux _ _ _
  _ = _                     := Fin_sum_range _
...
def goProp {α:Type} [OfNat α 0] [DecidableEq α] {β : Type} (go : β → α → α)
...
-- def QuadPvec {l:ℕ} (j : Fin l.succ) (moves: Vector (Fin 4) l) (ph : Vector Bool l.succ)
--   : Fin 4 → Fin l.succ → Prop  := λ a i ↦
--   goProp quad j moves ph a i

-- theorem point_earned_iff_QuadP {l:ℕ} (j: Fin l.succ) (moves: Vector (Fin 4) l)
-- (ph : Vector Bool l.succ)
-- (i: Fin l.succ):
-- (∃ a, goProp quad j moves ph a i)
-- ↔
-- point_earned quad (pathVec quad moves) i j ph
-- := by
--   rw [point_earned_iff_ex]
--   unfold point_property goProp
--   constructor
--   intro h; rcases h with ⟨a,ha⟩; exists a; simp;       tauto
--   intro h; rcases h with ⟨a,ha⟩; exists a; simp at ha; tauto

theorem pt_earned_iff_goProp
...
(∃ a, goProp go j moves ph a i) ↔ pt_earned go (pathVec go moves) i j ph
:= by rw [pt_earned_iff_ex]; unfold pt_property goProp; simp; tauto
...
theorem unique_loc_quad {l:ℕ} (j: Fin l.succ)
  ...
  ∀ (a : Fin 4) (i₀ i₁ : Fin l.succ) (_ : goProp quad j moves ph a i₀) (_ : goProp quad j moves ph a i₁),
  i₀ = i₁
...
theorem unique_loc_dir_quad {l:ℕ} (j: Fin l.succ)
  ...
  (_ : goProp quad j moves ph a₁ i),
  a₀ = a₁
) := And.intro (unique_loc_quad _ _ _ path_inj) (unique_dir_quad _ _ _)

-- theorem flip_loc_dir
-- {l:ℕ} (j: Fin l.succ) (moves: Vector (Fin 4) l) (ph : Vector Bool l.succ)
-- [DecidablePred fun a => ∃ i, goProp quad j moves ph a i]
-- [DecidablePred fun i => ∃ a, goProp quad j moves ph a i]
-- [(a : Fin 4) → DecidablePred fun n => ∃ (hq : n < Nat.succ l), goProp quad j moves ph a { val := n, isLt := hq }]
-- (path_inj: Function.Injective (λ k : Fin l.succ ↦ (pathVec quad moves).get k))
-- : Finset.card (Finset.filter (λ a ↦ ∃ i, goProp quad j moves ph a i) Finset.univ) =
--   Finset.card (Finset.filter (λ i ↦ ∃ a, goProp quad j moves ph a i) Finset.univ)
-- := choice_pack_finset_card (unique_loc_quad j moves ph path_inj)
--                            (unique_dir_quad j moves ph)


theorem pts_at_lin_Vec
  ...
  (path_inj : (Function.Injective fun k => Vector.get (pathVec quad moves) k))
  : pts_at' quad j ph (pathVec quad moves) ≤ 4 := by
    unfold pts_at'
    have : Finset.filter (
        λ i : Fin (Nat.succ l) ↦ (∃ a, goProp quad j moves ph a i)) Finset.univ =
          Finset.filter (
        λ i : Fin (Nat.succ l) ↦  pt_earned quad (pathVec quad moves) i j ph) Finset.univ
    := by
      apply Finset.ext;intro i;repeat (rw [Finset.mem_filter]);simp;
      exact pt_earned_iff_goProp quad j moves ph i
    rw [← this,← choice_pack_finset_card (unique_loc_dir_quad j moves ph path_inj)]
    simp_rw [← Finset.card_fin 4]
    apply card_finset_fin_le

theorem pts_earned_bound_dir
{l:ℕ} (ph : Vector Bool l.succ)
(moves: Vector (Fin 4) l)
[∀ i, DecidablePred fun i_1 : Fin l.succ => ∃ a : Fin 4, goProp quad i moves ph a i_1]
[∀ i, DecidablePred fun a                => ∃ i_1,       goProp quad i moves ph a i_1]
[∀ i, (a : Fin 4) → DecidablePred fun n => ∃ (hq : n < Nat.succ l),
                                                         goProp quad i moves ph a ⟨n, hq⟩]
(path_inj : (Function.Injective fun k => Vector.get (pathVec quad moves) k))

:
pts_tot quad ph (pathVec quad moves) ≤ l.succ * 4 -- Yes!
-- February 17, 2024. Next we could generalize from quad to go
:= by
  suffices pts_tot quad ph (pathVec quad moves) ≤ ∑ j : Fin l.succ, 4 by simp at this; tauto
  apply Finset.sum_le_sum
  intro i; intro
  rw [pts_at_eq_pts_at']
  exact pts_at_lin_Vec i ph moves path_inj


theorem pts_earned_bound
-- February 17, 2024
{l:ℕ} (ph : Vector Bool l.succ)
(moves: Vector (Fin 4) l)
[∀ i, DecidablePred fun i_1 : Fin l.succ => ∃ a : Fin 4, goProp quad i moves ph a i_1]
-- try to prove these instead of leaving them like this
[∀ i, DecidablePred fun a                => ∃ i_1,       goProp quad i moves ph a i_1]
[∀ i, (a : Fin 4) → DecidablePred fun n => ∃ (hq : n < Nat.succ l),
                                                         goProp quad i moves ph a ⟨n, hq⟩]
(path_inj : (Function.Injective fun k => Vector.get (pathVec quad moves) k))
...
pts_tot quad ph (pathVec quad moves) ≤ min (l.succ * 4) (l.succ * l / 2)

:= by
  suffices (
    pts_tot quad ph (pathVec quad moves) ≤ l.succ * 4
    ∧ pts_tot quad ph (pathVec quad moves) ≤ l.succ * l / 2) by
    exact Nat.le_min.mpr this
  constructor
  exact pts_earned_bound_dir ph moves path_inj
  exact pts_earned_bound_loc quad ph (pathVec quad moves)

-- example (l:ℕ) :
-- min (l.succ * 4) (l.succ * l / 2)
-- = l.succ * (min 8 l) / 2 := by
--   unfold min
--   sorry
...
∃ q, pts_tot_bound go ph q := by
  ...
  unfold pts_tot_bound
  intro moves
  let Q := pts_earned_bound_aux go ph (⟨(path go moves.1).1,path_len _ _⟩)
  tauto
