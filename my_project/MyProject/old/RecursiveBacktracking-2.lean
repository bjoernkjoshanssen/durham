...
def sqf_suf {k:ℕ} (w : Vector (Fin 3) (5-k)) : ℕ :=
by {
  induction k
  -- Base case:
...
theorem verify (k:ℕ) (bound : k ≤ 5)  (w : Vector (Fin 3) (5-k)):
...
( squarefree v ∧ g₂.1 <:+ v.1) } := fincard_ext _ _ this
  ...
  have : 5 < 5 := calc
    _ ≤ n := h
    _ < n.succ := by exact Nat.lt.base n
    _ ≤ 5 := bound
