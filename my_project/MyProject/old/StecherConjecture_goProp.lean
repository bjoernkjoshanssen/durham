...
pts_at go₁ k ph fold := by
  ...
  simp
  simp at ha
  exact pt_loc_of_embeds h_embed fold ⟨a,finfin a⟩ k ph ha
...
theorem unique_loc_quad' {l:ℕ} (j: Fin l.succ)
  ...
  ∀ (a : Fin 4) (i₀ i₁ : Fin l.succ) (_ : bond_path quad j moves ph a i₀) (_ : bond_path quad j moves ph a i₁),
  i₀ = i₁
  := λ a i₀ i₁ hai₀ hai₁ ↦ by
...
theorem unique_loc_quad {l:ℕ} (j: Fin l.succ)
  (moves: Vector (Fin 4) l) (ph : Vector Bool l.succ)
  (path_inj: Function.Injective (λ k : Fin l.succ ↦ (pathVec quad moves).get k)):
  ∀ (a : Fin 4) (i₀ i₁ : Fin l.succ) (_ : pt_dir quad j moves ph a i₀) (_ : pt_dir quad j moves ph a i₁),
  i₀ = i₁
...
theorem pts_at'_dir
  ...
  [DecidablePred fun i => ∃ a, pt_dir quad j moves ph a i]
  [DecidablePred fun a => ∃ i, pt_dir quad j moves ph a i]
  [ (a : Fin 4) → DecidablePred fun n => ∃ (hq : n < Nat.succ l), pt_dir quad j moves ph a { val := n, isLt := hq }]

  (path_inj : (Function.Injective fun k => Vector.get (pathVec quad moves) k))
  : pts_at' quad j ph (pathVec quad moves) ≤ 4 := by
    unfold pts_at'
    have : Finset.filter (
        λ i : Fin (Nat.succ l) ↦ (∃ a, pt_dir quad j moves ph a i)) Finset.univ =
          Finset.filter (
        λ i : Fin (Nat.succ l) ↦  pt_loc quad (pathVec quad moves) i j ph) Finset.univ
    := by
...
(moves: Vector (Fin 4) l)
[∀ i, DecidablePred fun i_1 : Fin l.succ => ∃ a : Fin 4, pt_dir quad i moves ph a i_1]
[∀ i, DecidablePred fun a                => ∃ i_1,       pt_dir quad i moves ph a i_1]
[∀ i, (a : Fin 4) → DecidablePred fun n => ∃ (hq : n < Nat.succ l),
                                                         pt_dir quad i moves ph a ⟨n, hq⟩]
(path_inj : (Function.Injective fun k => Vector.get (pathVec quad moves) k))
...
:= by
  ...
  intro i; intro
  rw [pts_at_eq_pts_at']
  exact pts_at'_dir i ph moves path_inj
...
theorem pts_earned_bound'
-- February 18, 2024. This means we don't need pts_at_eq_pts_at' at all.
{l:ℕ} (ph : Vector Bool l.succ)
(moves: Vector (Fin 4) l)
[∀ i, DecidablePred fun i_1 : Fin l.succ => ∃ a : Fin 4, pt_dir quad i moves ph a i_1]
-- try to prove these instead of leaving them like this
