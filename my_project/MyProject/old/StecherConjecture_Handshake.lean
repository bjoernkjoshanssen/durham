...
-- maybe make the definition more like this pathM (pathmatch):
...
-- #eval pathM rect ![0,2,1] -- works! but is it good for proving things?
-- #print pathM.loop
-- #print pathM
...
-- this may be the approach we've been looking for

-- theorem pathFF'
--   {α β:Type} [OfNat α 0] {l:ℕ} (go : β → α → α)
--   (moves : Fin l → β) (i: Fin l.succ):
--   pathF go moves i = pathF' go moves i
--   := by
--     induction l
--     have : i = 0 := by exact Fin.eq_zero i
--     subst this
--     rfl
--     unfold pathF pathf
--     unfold pathF'
--     simp
--     -- looks like pathF is preferable to pathF'
--     sorry
...
def morF'' {l:ℕ} -- 3/9/24
  ...
  intro i
  exact morF' f go moves i.1

-- should probably do morF without infinity just like pathF' was an improvement
-- of pathF:
...
:= Finset.card (
  Finset.filter
  (λ ik : (Fin l) × (Fin l) ↦ pt_loc go fold ik.1 ik.2 ph)
  (Finset.univ)
)   -- think "ik = (i,k)"

-- we may prove that twice the size of this is obtained without the i.succ < j
-- requirement by using a disjoint union
...
-- 3/14/2024. Now we want a 1:1 map from the LHS to pairs (i,a) such that there's a point at i
-- in direction a. Actually the map can work fiberwise.
-- need a map (fold j ↦ a), where go a fold i = fold j
-- fix x and consider y ↦ a where go a x = y.
-- So it is the inverse of a ↦ go a x
-- Function.invFun (λ a ↦ go a x) or
-- j ↦ Function.invFun (λ a ↦ go a (fold i)) (fold j)

-- First attempt: noncomputable since it uses invFun
noncomputable def handshake_map₁  {α:Type} {β : Type} [Fintype β]
[Nonempty β]
(go : β → α → α) [DecidableEq α]
 {l : ℕ} (fold : Vector α l) (phobic : Vector Bool l) (i: Fin l)
(hP: Symmetric (λ i j : Fin l ↦ phobic.get i && phobic.get j && nearby go (fold.get i) (fold.get j))):
    filter (
    λ j : (Fin l) ↦
    phobic.get i && phobic.get j && nearby go (fold.get i) (fold.get j) && (
         i.1.succ < j.1
      ∨ j.1.succ < i.1
    )
) univ
→     β
:= λ j ↦ Function.invFun (λ a ↦ go a (fold.get i)) (fold.get j.1)

-- Second attempt: uses option types
def handshake_map₂  {α:Type} {b:ℕ}
(go : Fin b → α → α) [DecidableEq α]
 {l : ℕ} (fold : Vector α l) (phobic : Vector Bool l) (i: Fin l)
-- (hP: Symmetric (λ i j : Fin l ↦ phobic.get i && phobic.get j && nearby go (fold.get i) (fold.get j)))
: Fin l → Option (Fin b)
:= λ j ↦ Fin.find  (λ a ↦ go a (fold.get i) = fold.get j)

-- It works, at least:
#eval (handshake_map₂
  rect
  (⟨[(0,0),(0,1)],rfl⟩ : Vector (ℤ×ℤ) 2)
  (⟨[true,true],rfl⟩ : Vector (Bool) 2)
  1 -- i
  ⟨0,Nat.two_pos⟩ -- j
)

-- Third attempt: use option type, but eliminate it.
noncomputable def handshake_map₃  {α:Type} {b:ℕ}
(go : Fin b → α → α) [DecidableEq α]
 {l : ℕ} (fold : Vector α l)
 (phobic : Vector Bool l) (i: Fin l)
:
    filter (
    λ j : (Fin l) ↦
    phobic.get i && phobic.get j && nearby go (fold.get i) (fold.get j) && (
...
:= λ j ↦ by
  ...
  unfold nearby at Q
  simp at Q
  let v := Fin.find (λ a ↦ fold.get j = go a (fold.get i))
  have : Option.isSome v := Fin.isSome_find_iff.mpr Q
  exact v.get this
-- to describe phobic.get j from a, use
-- phobic j = phobic (foldInv (go a (fold.get i)))




noncomputable def handshake_map_3_to_2  {α:Type} {b:ℕ}
  ...
  :
  filter (
    λ iaj : ((Fin l.succ) × (Fin b)) × (Fin l.succ) ↦ -- iaj = ((i,a),j)
             phobic.get iaj.1.1 && phobic.get iaj.2
    && nearby go (fold.get iaj.1.1) (fold.get iaj.2)
      && (
         iaj.1.1.1.succ < iaj.2.1
                     ∨ iaj.2.1.succ < iaj.1.1.1
    )
  ) univ
→  filter (
    λ ia : ((Fin l.succ) × (Fin b)) ↦ -- ia = (i,a)
    ∃ j : Fin l.succ,
             phobic.get ia.1 && phobic.get j
    && nearby go (fold.get ia.1) (fold.get j)
      && (
         ia.1.1.succ < j.1
                     ∨ j.1.succ < ia.1.1
    )
...
noncomputable def handshake_map₄  {α:Type} {b:ℕ}
-- finished 3/15/24
-- want to prove that this map is 1:1
  (go : Fin b → α → α) [DecidableEq α]
  {l : ℕ} (fold : Vector α l.succ)
  (hfold: Function.Injective (λ i : Fin l.succ ↦ ((fold.get i) : α)))
  (timeAt : α → Fin l.succ) (htimeAt : Function.LeftInverse  (λ j ↦ fold.get j) timeAt)
  -- timeAt is actually an unrealistic assumption since we are not doing a space-filling curve.
  -- so let's map (i,j) to (i,j,a) and then drop the j.
  (phobic : Vector Bool l.succ)
  :
    filter (
    λ ij : (Fin l.succ) × (Fin l.succ) ↦
  ...
  filter (
    λ ia : (Fin l.succ) × (Fin b) ↦ -- ia = (i,a)
             phobic.get ia.1 && phobic.get (timeAt (go ia.2 (fold.get ia.1)))
    && nearby go (fold.get ia.1) (fold.get (timeAt (go ia.2 (fold.get ia.1))))
      && (
         ia.1.1.succ < (timeAt (go ia.2 (fold.get ia.1))).1
                     ∨ (timeAt (go ia.2 (fold.get ia.1))).1.succ < ia.1.1
    )
  ) univ
:= λ ij ↦ by
  simp at ij
  ...
  exact ⟨⟨i,a⟩,by
    simp
    have hf : ∀ x, fold.get (timeAt x) = x := htimeAt
    have h₀: fold.get (ij.1.2)
      = fold.get (timeAt (go a (Vector.get fold (ij.1).1)))
      := by
        rw [hf]
        rw [Fin.find_spec
          (λ a : Fin b ↦ fold.get ij.1.2 = go a (fold.get ij.1.1))
...
-- theorem handshakemap₄injective : Function.Injective (handshake_map₄) := by
--   intro h
--   sorry

-- def that_map  {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
--  {l : ℕ} (fold : Vector α l) (phobic : Vector Bool l)
-- (hP: Symmetric (λ i j : Fin l ↦ phobic.get i && phobic.get j && nearby go (fold.get i) (fold.get j))):
--     filter (
--     λ a : (Fin l) × (Fin l) ↦
...
-- --     filter (
-- --     λ a : (Fin l) × β ↦
-- --     phobic.get a.1 && phobic.get a.2 && go (fold.get a.1) = (fold.get a.2) && (
-- --          a.1.1.succ < a.2.1
-- --       ∨ a.2.1.succ < a.1.1
-- --     )
-- -- ) univ
...
points_tot go ph fold = pts_tot' go ph fold := by
  ...
  := by
    ...
    let f := (λ ik : (Fin l) × (Fin l) ↦ (λ h : ik ∈ s' ↦ ik.1))
...
def pts_totF {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
 {l:ℕ} (ph : Fin l → Bool)(fold : Fin l → α)
  := ∑ i : {i₀ : Fin l // ph i₀}, pts_at'F go i ph (fold)
...
theorem pts_bound_of_embedsF {α:Type} [DecidableEq α] {b₀ b₁ : ℕ}
{go₀ : Fin b₀ → α → α} {go₁ : Fin b₁ → α → α} (h_embed : embeds_in go₀ go₁)
{l:ℕ} (ph : Fin l → Bool) (fold : Fin l → α):
pts_totF go₀ ph fold ≤
pts_totF go₁ ph fold := by
  unfold pts_totF; apply Finset.sum_le_sum; intros; exact pts_at_of_embeds'F h_embed _ _ _
...
: pts_tot'F go ph (pathF go moves) ≤ l.succ * b := by
  suffices pts_tot'F go ph (pathF go moves) ≤ ∑ j : Fin l.succ, b by
    simp at this; tauto
  apply Finset.sum_le_sum; intro i; intro
...
-- #eval "f"
-- #eval Nat.fold (f ![0,0,0,0,0]) 0 0
-- #eval Nat.fold (f ![0,0,0,0,0]) 1 0
-- #eval Nat.fold (f ![0,0,0,0,0]) 2 0
-- #eval Nat.fold (f ![0,0,0,0,0]) 3 0
-- #eval Nat.fold (f ![0,0,0,0,0]) 4 0
-- #eval Nat.fold (f ![0,0,0,0,0]) 5 0

-- def pathF {l:ℕ} {α: Type}{b:ℕ} [OfNat α 0] (go : Fin b → α → α) (moves : Fin l → β) :
-- Fin l.succ → α := by
--   intro i

--   sorry
-- theorem path_succ  {l :ℕ} (i:Fin l)
-- (moves : Vector (Fin 4) l.succ) :
-- (pathᵥ rect moves).get (⟨i.1.succ,by
--   sorry
-- ⟩)  = rect (moves.get ⟨i.1,sorry⟩) ((pathᵥ rect moves).get ⟨i.1,sorry⟩) := by
--   -- use "take" and cons
--   sorry
...
lemma pathF'_dif_pos'_inductive
-- This is a very weird lemma. Probably not useful.
{l: ℕ}
(i: Fin (l))
(moves: Fin ((Nat.succ l)) → Fin 4)
(h': pathF' rect (λ j ↦ moves (Fin.castSucc j)) ⟨i.1 + 1, Nat.add_lt_of_lt_sub i.2⟩
   = rect (moves (Fin.castSucc i))
   (pathF' rect (λ j ↦ moves (Fin.castSucc j)) ⟨ i.1, Nat.lt.step i.2⟩))
:    pathF' rect moves                          ⟨i.1 + 1, Nat.succ_lt_succ_iff.mpr (Nat.lt.step i.2) ⟩
   = rect (moves i)
    (pathF' rect moves                          ⟨i.1, Nat.lt.step (Nat.lt.step i.2)⟩)
:= by
  unfold pathF'
  simp
  have : i.1 < l.succ := Nat.lt.step i.2
  rw [dif_pos this]
  have : i.1+1 < l.succ := Nat.add_lt_of_lt_sub i.2
  rw [dif_pos this]
  let g := Nat.rec (motive := fun {l} => (Fin l → Fin 4) → Fin (Nat.succ l) → ℤ × ℤ)
    (fun moves a => 0)
    (fun n n_ih moves i =>
      if h : ↑i < Nat.succ n then n_ih (fun j => moves (Fin.castSucc j))
      ⟨i.1,h⟩
      else rect (moves (Fin.last n)) (n_ih (fun j => moves (Fin.castSucc j)) (Fin.last n)))
    l (fun j => moves (Fin.castSucc j))
  have hg: g = pathF' rect (λ j ↦ moves (Fin.castSucc j)) := by
    unfold pathF'
    simp
  have h:                            g ⟨ i.1 + 1, this⟩
    = rect (moves (Fin.castSucc i)) (g ⟨ i.1, Nat.lt.step i.2⟩) := by
    rw [hg]
    exact h'
  exact h
...
:= by
  ...
  have h:                            g ⟨ i.1 + 1, this⟩
    ...
    symm
    sorry
  have h': pathF' rect (λ j ↦ moves (Fin.castSucc j)) ⟨ i.1 + 1, this⟩
    = rect (moves (Fin.castSucc i)) ((pathF' rect (λ j ↦ moves (Fin.castSucc j))) ⟨ i.1, Nat.lt.step i.2⟩) := by
    symm
    sorry
  exact h
...
lemma pathF'_dif_pos -- i+2,i+1,i+1
{l: ℕ}
(i: Fin (Nat.succ l))
(moves: Fin (Nat.succ (Nat.succ l)) → Fin 4)
: pathF' rect moves { val := i.1 + 2, isLt := (by
  ...
  rect (moves (Fin.succ i))
    (pathF' rect moves { val := i.1 + 1, isLt := (by
      ...
      : i.1 + 1 < l.succ.succ.succ) })
:= by
  let Q := Fin.cons_self_tail moves
  -- should use Fin.find to find the first occurrence of 2
  unfold pathF'
  simp
  -- I guess pathF' was defined a bit differently
  -- have : i+2 < l.succ.succ := sorry
...
theorem orderly_injective'F {l :ℕ} (i:Fin l.succ)
(moves : Fin l.succ.succ → (Fin 4)) (h: moves (Fin.castSucc i) = 0 ∧ moves (Fin.succ i) = 1) :
¬ Function.Injective (λ i ↦ (pathF' rect moves) i) := by
  intro hc
  have : rect 1 (rect 0 ((pathF' rect moves) ⟨i.1, Nat.lt.step (Nat.lt.step i.2)⟩))
  ...
  ⟩)
       = (pathF' rect moves) ⟨i.1+2,by simp_rw [Nat.succ_eq_add_one];simp⟩ := by
    rw [← h.2];symm;
    exact pathF'_dif_pos i moves
  have h₀: rect 0 ((pathF' rect moves) ⟨i.1, Nat.lt.step (Nat.lt.step i.2)⟩)
       = (pathF' rect moves) ⟨i.1+1, Nat.succ_lt_succ_iff.mpr (Nat.lt.step i.2)
    ⟩ := by
    rw [← h.1];
    let Q := pathF'_dif_pos' i moves
    simp at Q
    rw [Q]
  have : (pathF' rect moves) ⟨i.1, Nat.lt.step (Nat.lt.step i.2)⟩
       = (pathF' rect moves) ⟨i.1+2,by simp_rw [Nat.succ_eq_add_one];simp⟩ := by
...
-- Crowning achievement of 3/9/24:
-- Should be combined with the "reflect" result "towards_orderly".
...
/- To imitate Python style we define FoldTask: -/
/- FoldTask is an instance of a Decision Problem or rather Function Problem  -/
structure FoldTask where
  moveSetSize : ℕ
  numMoves : ℕ
  space : Type
  origin : OfNat space 0
  equ : DecidableEq space

  go : Fin moveSetSize → space → space
  phobe : Vector Bool numMoves.succ

def myFoldTask : FoldTask :=
{
  moveSetSize := 4
  space := ℤ×ℤ
  numMoves := 3
  origin := Zero.toOfNat0
  equ := instDecidableEqProd
  go := rect
  phobe := ⟨[true,false,false,true],rfl⟩
}

def FoldTask.maxScore {α:Type} [OfNat α 0]
[DecidableEq α] : FoldTask → ℕ :=
  λ F ↦ by
    obtain : OfNat F.space 0 := F.origin
    obtain : DecidableEq F.space := F.equ
    exact HP F.go F.phobe

theorem python : myFoldTask.maxScore = 1 := by decide
--- would also want: myFoldTask.listOfOptimalFolds
-- End of Python style
...
--   (⟨[f,f,f,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,f,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,f,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,f,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,f,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,f,t,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,f,t,t,t,t],rfl⟩ : Vector Bool 6),

--   (⟨[f,t,f,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,f,t,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[f,t,t,t,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,f,t,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,f,t,t,t,t],rfl⟩ : Vector Bool 6),

--   (⟨[t,t,f,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,f,t,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,f,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,f,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,f,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,f,t,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,t,f,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,t,f,t],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,t,t,f],rfl⟩ : Vector Bool 6),
--   (⟨[t,t,t,t,t,t],rfl⟩ : Vector Bool 6),
...
:= by
  ...
  -- let R₂ := n_ih (λ i ↦ moves (Fin.castSucc i)) (Fin.castSucc (Fin.last n))
  -- unfold pathF' at R₂
  -- unfold morfF at R₂
  -- simp at R₂
  -- rw [R₂]
  -- have : (⟨n, Nat.lt.step (Nat.lt.base n)⟩ : Fin n.succ.succ)
  --       = (Fin.last n : Fin n.succ)  := sorry
...
theorem towards_orderlyish
  ...
  pts_tot' κ ph (π κ moves')
  := by
  cases can_be_orderlyish (moves.get 0)
  exists moves
  cases h
  exists (σ ρ κ moves)
  constructor
  ...
  exact rotate_pts_tot ph moves
  cases h_1
  exists (σ ρ κ (σ ρ κ moves))
  constructor
  rw [← h]
  simp
