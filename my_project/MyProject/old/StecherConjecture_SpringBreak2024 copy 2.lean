...
def Fin_trans' {l : ℕ} {k: Fin l} (i : Fin k): Fin l.pred :=
  ⟨i.1, Fin.val_lt_of_le _ (by
    refine Nat.le_pred_of_lt ?h
    exact k.2
  )⟩
...
def Fin_trans_strengthen {l : ℕ} {k: Fin l} (i : Fin k.1.pred): Fin l.pred :=
  ⟨i.1, by
    refine Fin.val_lt_of_le i ?h
    refine Nat.pred_le_pred ?h.a
    exact Fin.is_le'
  ⟩
...
def change_type_general
{l:ℕ} (k : Fin l) (P : Fin l → Fin l → Bool)
[DecidablePred fun i : Fin l => P i k]
[DecidablePred fun i : Fin k => P (Fin_trans i) k]
(h: ∀ x y : Fin l, P x y → x.1 < y.1):
  Finset.filter (λ i : Fin l ↦ (P i k)) (Finset.univ : Finset (Fin l))
→ Finset.filter (λ i : Fin k ↦ P (Fin_trans i) k) (Finset.univ : Finset (Fin k))
  := by
    intro ip;let i₃ := ip.2;simp at i₃;simp;exact ⟨(⟨ip.1.1,h _ _ i₃⟩ : Fin k),i₃⟩
...
→ Finset.filter (λ i : Fin k.1.pred ↦ P (Fin_trans'' i) k) (Finset.univ : Finset (Fin k.1.pred))
  := by
    intro ip;let i₃ := ip.2;simp at i₃;simp;
    exact ⟨(⟨ip.1.1,h _ _ i₃⟩ : Fin k.1.pred),i₃⟩
...
=
Fintype.card (Finset.filter (λ i : Fin k ↦ (P (Fin_trans i) k)) Finset.univ)
:= by
  suffices Function.Bijective (change_type_general k P h) by
    apply Fintype.card_of_bijective; exact this
  constructor
  . intro i₁ i₂ hi; unfold change_type_general at hi; simp at hi
    exact SetCoe.ext (Fin.ext hi)
  . intro i;
    exists ⟨Fin_trans i, by let Q := i.2;simp at Q;simp;exact Q⟩
    apply Subtype.ext;apply Fin.ext;unfold change_type_general Fin_trans;simp

theorem change_type_card_general'' {l:ℕ} (k : Fin l) (P : Fin l → Fin l → Bool)
[DecidablePred fun i : Fin l => P i k]
[DecidablePred fun i : Fin k.1.pred => P (Fin_trans'' i) k]
(h: ∀ x y : Fin l, P x y → x.1.succ < y.1)
:
Fintype.card (Finset.filter (λ i : Fin l ↦ P i k) Finset.univ)
=
...
-- def change_type  {α β : Type} [DecidableEq α] [Fintype β] (go : β → α → α)
-- {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l):
--   Finset.filter (λ i : Fin l ↦ (pt_loc go fold            i  k ph)) Finset.univ
-- → Finset.filter (λ i : Fin k ↦ (pt_loc go fold (Fin_trans i) k ph)) Finset.univ
--   := by
--     intro ip;
--     let Q := ip.2;
--     simp at Q
--     -- rw [Finset.mem_filter] at Q -- Finset.mem_filter was key here
--     unfold pt_loc at Q;
--     have : ip.1.1.succ < k := by
--       simp at Q;tauto
--     exact ⟨⟨ip.1.1,Nat.lt_of_succ_lt this⟩,by rw [Finset.mem_filter];simp;tauto⟩

/-- Nicer proof than `change_type` but somehow doesn't work with `change_type_card`.-/
-- def change_type'  {α β : Type} [DecidableEq α] [Fintype β] (go : β → α → α)
-- {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l):
--   Finset.filter (λ i : Fin l ↦ (pt_loc go fold            i  k ph)) Finset.univ
-- → Finset.filter (λ i : Fin k ↦ (pt_loc go fold (Fin_trans i) k ph)) Finset.univ
...
--       simp at Q;tauto
--     simp at Q
--     exact ⟨⟨ip.1.1,Nat.lt_of_succ_lt this⟩,Q⟩
...
-- =
-- Fintype.card (Finset.filter (λ i : Fin k ↦ (pt_loc go fold (Fin_trans i) k ph)) Finset.univ)
-- := by
...
--   . intro i;
--     exists ⟨Fin_trans i, by let Q := i.2;simp at Q;simp;exact Q⟩
--     apply Subtype.ext;apply Fin.ext;unfold change_type' Fin_trans;simp
...
theorem go_WS_injective : Function.Injective go_WS := by
  ...
  . --pos
    ...
    . -- pos
      rw [if_pos hy] at hxy; exact sp_injective hxy
    -- neg
    rw [if_neg hy] at hxy; unfold sp sm at hxy
...
def pathD'  {α : Type} {l:ℕ} [OfNat α 0] {β : Type} (go : β → α → α) (moves : Vector β l) :
Fin l.succ → α
...
def path₀ {α : Type} [OfNat α 0] {β : Type} (go : β → α → α) (moves : Vector β l) (i: Fin l.succ)
: α
...
--   exact 0
--   intro moves
--   let firstMoves := Fin.init moves
--   let lastMove := moves (Fin.last n)
--   -- exact Fin.cons (go lastMove (n_ih firstMoves (Fin.last n))) (n_ih firstMoves)
--   exact Matrix.vecCons (go lastMove (n_ih firstMoves (Fin.last n))) (n_ih firstMoves)
...
:= by
      ...
      have : ik ∈ slicer P k₁ := by exact hA₁ hikA
      have h₁: ik.2 = k₁ ∧ P ik.1 k₁ = true := by unfold slicer at this;simp at this;exact this
      have : ik ∈ slicer P k₂ := by exact hA₂ hikA
      have h₂: ik.2 = k₂ ∧ P ik.1 k₂ = true := by unfold slicer at this;simp at this;exact this
...
:= by
    intro x;repeat rw [← Fintype.card_coe];simp
    let f : {ik : Fin l × Fin l // ik.2 = x ∧ P ik.1 x} → { i_1 : Fin l // P i_1 x}
      := λ ik ↦ ⟨ik.1.1, ik.2.2⟩
    have : Function.Bijective f := by
      constructor;intro x y hxy;simp at hxy
      have : x.1 = y.1 := by apply Prod.ext;exact hxy;rw [x.2.1,y.2.1]
      exact SetCoe.ext this;intro y;exists ⟨⟨y.1,x⟩,by simp;exact y.2⟩
    apply Fintype.card_of_bijective this

    -- old proof:
    -- intro x;let α' := (Fin l) × (Fin l)
    -- let s' := (Finset.filter (fun ik : (Fin l) × (Fin l) =>
    --   ik.2 = x ∧ P ik.1 x = true) Finset.univ
    -- )
    -- let t := (Finset.filter (fun i_1 : Fin l => P i_1 x = true) Finset.univ)
...
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l) : ℕ :=
  Finset.card (
    Finset.filter (λ i : Fin k ↦ (pt_loc go fold (Fin_trans i) k ph))
    Finset.univ
  )
...
-- pts_at' go i ph fold ≤ i := by -- can make this i.pred I think
--   rw [← pts_at_eq_pts_at', ← Finset.card_fin i.1];
...
:= by
  ...
  exact match c with
  | 0 => by apply Prod.ext;simp;decide;simp;rw [Int.add_comm];simp;decide
  | 1 => by apply Prod.ext;simp;decide;simp;rw [Int.add_comm];simp;decide
  | 2 => by apply Prod.ext;simp;decide;simp;rw [Int.add_comm];simp;decide
  | 3 => by apply Prod.ext;simp;decide;simp;rw [Int.add_comm];simp;decide
...
:= by
  ...
  exact match c with
  | 0 => by apply Prod.ext;simp;rw [Int.add_comm];simp;decide;simp;decide
  | 1 => by apply Prod.ext;simp;rw [Int.add_comm];simp;decide;simp;decide
  | 2 => by apply Prod.ext;simp;rw [Int.add_comm];simp;decide;simp;decide
  | 3 => by apply Prod.ext;simp;rw [Int.add_comm];simp;decide;simp;decide
...
:= by
  ...
  . -- cons
    ...
    by_cases h : k = 0
    . -- pos
      subst h;simp;rw [reflect_basic];let Q := tail_ih 0;simp at Q;exact congr_arg _ Q
    . -- neg
      obtain ⟨s,hs⟩ := Fin.eq_succ_of_ne_zero h
      subst hs
      simp
...
:= by
  ...
  . -- cons
    ...
    by_cases h : k = 0
    . -- pos
      subst h;simp;rw [rotate_basic];let Q := tail_ih 0;simp at Q;exact congr_arg _ Q
    . -- neg
      obtain ⟨s,hs⟩ := Fin.eq_succ_of_ne_zero h
      subst hs
      simp
...
:= by obtain ⟨a,⟨u,hu⟩⟩ := Vector.exists_eq_cons moves;rw [hu, Vector.head_cons];rfl
...
theorem towards_orderlyish
  ...
  . -- inr
    ...
    . -- inr.inl
      exists m₁
      constructor
      . simp;rw [← h_1];simp;symm;simp at h_1;exact rotate_head _
...
def equifoldable (go : Fin 4 → ℤ×ℤ → ℤ×ℤ)
  {l:ℕ} (ph₀ ph₁ : Vector Bool l.succ.succ) (p:ℕ) :=
...
instance (go : Fin 4 → ℤ×ℤ → ℤ×ℤ)
  {l:ℕ}  (p:ℕ) : Equivalence (λ (ph₀ ph₁ : Vector Bool l.succ.succ) ↦ equifoldable go ph₀ ph₁ p)
    := {
      trans := by
...
instance (go : Fin 4 → ℤ×ℤ → ℤ×ℤ)
  {l:ℕ} (ph₀ ph₁ : Vector Bool l.succ.succ) (p:ℕ) : Decidable (equifoldable go ph₀ ph₁ p) := by
  unfold equifoldable
  unfold goodFolds
...
(go: β → α→α) {l:ℕ} (ph₀ ph₁ : Vector Bool l.succ) (hsub:
  toSet ph₀ ⊆ toSet ph₁) :
    ...
    have h₀ : (Finset.sum Finset.univ fun i => pts_at' go i ph₀ ⟨ (path go moves.1).1, by rw [path_len]⟩)
      ...
      have h_ : j ∈ Finset.filter (fun i' => Vector.get ph₀ i' = true) Finset.univ
              ∧ i ∈ Finset.filter (fun i' => Vector.get ph₀ i' = true) Finset.univ := by simp;exact hj.1.1
...
theorem more_pts_of_subset (go: Fin 4 → ℤ×ℤ→ℤ×ℤ) {l:ℕ} {ph₀ ph₁ : Vector Bool l.succ.succ}
 ...
 :  pts_tot'_list_rev' go       ph₀.1 w.1 ≤ pts_tot'_list_rev' go ph₁.1 w.1 := by
      ...
      have hi': ⟨i.1,hi⟩ ∈ Finset.filter (fun i => Vector.get ph₀ i = true) Finset.univ := by simp; tauto
      have hj': ⟨j.1,hj⟩ ∈ Finset.filter (fun i => Vector.get ph₀ i = true) Finset.univ := by simp; tauto
      unfold toSet at hsub
      let hsubj := hsub hj';simp at hsubj;
      let hsubi := hsub hi';simp at hsubi;
      tauto


def meet {l:ℕ} (ph₀ ph₁ : Vector Bool l) : Vector Bool l :=
  Vector.ofFn (λ i ↦ ph₀.get i ∧ ph₁.get i)
...
theorem meet_basic₀ {l :ℕ} {ph₀ ph₁ : Vector Bool l}
  : toSet (ph₀ ⊓ ph₁) ⊆ toSet ph₀ := by
  intro i hi;unfold toSet at *;simp at *;rw [meet_get] at hi;tauto


theorem meet_basic₁
  {l :ℕ} {ph₀ ph₁ : Vector Bool l}
  : toSet (ph₀ ⊓ ph₁) ⊆ toSet ph₁ := by
...
theorem goodFolds_monotone
  (go: Fin 4 → ℤ×ℤ→ℤ×ℤ) {l :ℕ} (ph₀ ph₁ : Vector Bool l.succ.succ)
 ...
 : goodFolds go p ph₀ ⊆ goodFolds go p ph₁ := by
  ...
  have hw₁ : w ∈ Finset.filter (fun v =>
    (Function.Injective fun i => Vector.get (path go v.1) i) ∧
    (p ≤ pts_tot'_list_rev' go ph₁.1 v.1 ∧ orderly_and_nontrivial v.1) ∧ (Gap_nil' 4 (Nat.succ l)).1 <:+ v.1
  ) Finset.univ := by simp;tauto
  rw [← verify₁] at hw₁
  tauto
...
/-- This is an obvious consequence of earlier results.-/
-- theorem not_quite_a_meet_homomorphism
--   (go: Fin 4 → ℤ×ℤ→ℤ×ℤ) {l p:ℕ} (ph₀ ph₁ : Vector Bool l.succ.succ)
--   -- this can be used to prove convexity of ∼ : if toSet ph₀ ⊆ toSet ph₁ ⊆ toSet ph₂
--   -- and ph₀ ∼ ph₂ then ph₀ ∼ ph₁.
--  : goodFolds go p (meet ph₀ ph₁) ⊆
--    goodFolds go p ph₀
--    ∩
--    goodFolds go p ph₁
--    -- the converse if false as ph₀ = 01011010, ph₁ = 00111100, p=2 shows
--    -- see a_meet_homomorphism however.
--    := by
-- intro w hw
-- simp
-- constructor
-- apply a_certain_homomorphism go (meet ph₀ ph₁)
-- exact meet_basic₀
-- apply a_certain_homomorphism go (meet ph₀ ph₁)
-- trivial
-- tauto
...
def goodPairs -- points_tot = Fin.card points_loc
  (go: Fin 4 → ℤ×ℤ→ℤ×ℤ) {l : ℕ} (fold : Vector (ℤ×ℤ) l) (ph : Vector Bool l)
...
theorem goodPairs_meet
  (go: Fin 4 → ℤ×ℤ→ℤ×ℤ) {l : ℕ} (ph₀ ph₁ : Vector Bool l.succ)
 ...
 : goodPairs go fold (ph₀ ⊓ ph₁) =
   ...
   goodPairs go fold ph₁ := by
    ...
    . -- mp
      ...
      unfold pt_loc at *
      simp at *
      have hi: ij.1 ∈ Finset.filter (fun i => Vector.get (meet ph₀ ph₁) i = true) Finset.univ
        := by simp; exact hij.1.1.1
      have hj: ij.2 ∈ Finset.filter (fun i => Vector.get (meet ph₀ ph₁) i = true) Finset.univ
        := by simp; exact hij.1.1.2
