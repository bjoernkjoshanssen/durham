...
--   {l:ℕ} (j : Fin l.succ) (ph : Vector Bool l.succ) (moves: Vector (Fin 4) l)
...
-- theorem pts_earned_bound'_tri {l:ℕ} (ph : Vector Bool l.succ) (moves: Vector (Fin 3) l)
-- (path_inj : (Function.Injective fun k => Vector.get (pathVec tri moves) k))
-- : pts_tot' tri ph (pathVec tri moves) ≤ min (l.succ * 3) (l.succ * l / 2)
-- :=
-- pts_earned_bound' ph moves path_inj right_injective_tri left_injective_tri

-- theorem pts_earned_bound'_hex {l:ℕ} (ph : Vector Bool l.succ) (moves: Vector (Fin 6) l)
-- (path_inj : (Function.Injective fun k => Vector.get (pathVec hex moves) k))
...
-- theorem pts_earned_bound'_rect {l:ℕ} (ph : Vector Bool l.succ) (moves: Vector (Fin 4) l)
-- (path_inj : (Function.Injective fun k => Vector.get (pathVec rect moves) k))
-- :
...
--     let Q := choice_ex_finset_card 4 l (QuadP k moves ph) R S
--     unfold QuadP at Q
--     simp at Q
--     unfold point_earned
--     simp
...
: P_tri' ph ≤ l.succ * 3 := by
  suffices pts_tot_bound tri ph (l.succ * 3) by
...
: P_hex' ph ≤ l.succ * 6 := by
  suffices pts_tot_bound hex ph (l.succ * 6) by
    exact Nat.find_le this
  unfold pts_tot_bound
  intro moves
  intro path_inj
...
: P_rect' ph ≤ l.succ * 4 := by
  suffices pts_tot_bound rect ph (l.succ * 4) by
    exact Nat.find_le this
  unfold pts_tot_bound
  intro moves
  intro path_inj
