...
∀ moves : Vector β l, Function.Injective (λ i ↦ (path go moves.1).get i) →
...
example {α : Type} (k l : ℕ) (u : Vector α k) (v : Vector α l) (h: u.1 = v.1)
(z : Fin k) : Vector.get u z = Vector.get v z := sorry

theorem preserve_inj
{α: Type} [OfNat α 0] [DecidableEq α]
{b₀ b₁: ℕ}
{go₀: Fin b₀ → α → α}
{go₁: Fin b₁ → α → α}
{l: ℕ}
{moves: Vector (Fin b₀) l}
(h_inj: Function.Injective fun i => Vector.get (path go₀ moves.1) i)
(f: Fin b₀ → α → Fin b₁)
(hf: is_embedding go₀ go₁ f)

:  Function.Injective fun i => Vector.get (path go₁ (
    ⟨
      morph f go₀ moves.1,
      (by rw [morph_len]; exact moves.2)
    ⟩ : Vector (Fin b₁) l).1) i := by
      let moves' := (
      ⟨
        morph f go₀ moves.1,
        (by rw [morph_len]; exact moves.2)
      ...
      intro x y hxy
      simp at hxy
      have : (path go₁ (morph f go₀ moves.1)).1 = (path go₀ moves.1).1 := by
        rw [← transform_of_embed f go₀ go₁ moves.1 hf]
        unfold path_transformed
        simp
      -- first establish that (path go₁ (morph f go₀ moves.1))
      -- and (path go₀ moves.1) have the same length, then build vectors from there
      have : (path go₁ (morph f go₀ moves.1)).length = (path go₀ moves.1).length := sorry
      have h_: ∀ z, Vector.get (path go₁ (morph f go₀ moves.1)) z =
                    Vector.get (path go₀ moves.1) z := by
        intro z
        -- need: Vector_get depends only on val
        ...
        simp
        sorry
      have : Vector.get (path go₀ moves.1) x
          = Vector.get (path go₀ moves.1) y := calc
        _ = Vector.get (path go₁ (morph f go₀ moves.1)) x := (h_ x).symm
        _ = Vector.get (path go₁ (morph f go₀ moves.1)) y := hxy
        _ = _ := h_ y
      let Q := h_inj this
      have : x.1 = y.1 := by
        norm_cast at Q
...
{l:ℕ} (ph : Vector Bool l.succ) : HP go₀ ph ≤ HP go₁ ph := by
  ...
  let moves' := (
    ⟨
      ...
      (by rw [morph_len]; exact moves.2)
    ⟩ : Vector (Fin b₁) l)
  have h_inj':  Function.Injective fun i => Vector.get (path go₁ moves'.1) i :=
    preserve_inj h_inj _ hf
