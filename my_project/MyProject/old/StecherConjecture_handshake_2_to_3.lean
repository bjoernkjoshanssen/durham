...
import MyProject.Handshake_aux
...
(we can always assume our fold starts by going to the right) (see `which_theorem?`)
we use the type
...
:= by
  ...
  _ = card ( filter (
    λ ia : (Fin l.succ) × (Fin b) ↦ -- ia = (i,a)
    phobic.get ia.1 && (∃ j : Fin l.succ,  phobic.get j
      && (fold.get j) = go ia.2 (fold.get ia.1)
      && (ia.1.1.succ < j.1 ∨ j.1.succ < ia.1.1))
    ) univ) := by
      congr;simp;apply funext;intro ab
      simp;constructor;intro h
      constructor;tauto;obtain ⟨j,hj⟩ := h
      exists j;tauto;intro h;obtain ⟨j,hj⟩ := h.2
      exists j;tauto
  _ ≤ _ := by
    simp
    exact card_product_set_type'' l.succ b (λ i ↦ phobic.get i) _
...
theorem independence_in_symmetric_pts_earned_bound_dir'₁ :
  ...
  intro hc
  have : nearby rect₃ (0,0) (0,1) := by decide
  let Q := hc this
  simp at Q
  unfold nearby at Q
  simp at Q
...
theorem independence_in_symmetric_pts_earned_bound_dir'₂ :
  ...
  simp at Q
  let R := @hc 0 0 1 rfl
  exact Fin.zero_ne_one R
  constructor

  intro hc
  unfold left_injective at hc
  let Q := hc 0
  unfold Function.Injective at Q
  let R := @Q (λ x ↦ x) (λ _ ↦ 0) rfl
  simp at R
  have : ∀ y : Fin 2, y = 0 := by
    intro y
    calc
