...
-- lemma2: if List.nodup fold.1 then there are at most Fincard β many such a.
-- lemma1: there are only b₀ nearby points
-- Finset.card Finset.filter (nearby) Finset.univ ≤ b₀

open BigOperators

def pts_at_with' {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l) : ℕ :=
  ∑ i : Fin k, nat_of_bool (point_achieved_with (go : β → α → α) fold ⟨i.1,finfin i⟩ k ph)


-- After thinking about linear bounds, realized this is a better version of pts_at_with':
def pts_at_with'' {α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector α l) : ℕ :=
  Finset.card (
    Finset.filter (λ i : Fin k ↦ (point_achieved_with (go : β → α → α) fold ⟨i.1,finfin i⟩ k ph))
    Finset.univ
  )
...
def neighbors  : Finset (ℤ×ℤ) := {
  val := Multiset.ofList [(1,0),(-1,0),(0,1),(0,-1)]
  nodup := by decide
}

def neighbors' (x:ℤ×ℤ)  : Finset (ℤ×ℤ) := {
  val := Multiset.ofList [(1,0),(-1,0),(0,1),ite (Even (x.1+x.2)) (0,-1) (0,0)]
  nodup := by
    by_cases h : (Even (x.1+x.2))
    rw [if_pos h]
    decide
...
theorem sf : Finset.card neighbors = 4 := by decide

def mylist := (⟨[(0,1),(2,2), (1,1), (1,-1),(1,0)],rfl⟩ : Vector (ℤ×ℤ) 5)

def mypoints (y:ℤ×ℤ) := Finset.card (Finset.filter (λ v ↦ y + v ∈ mylist.1) neighbors)

#eval mypoints (1,0)

theorem sfg (y:ℤ×ℤ) : mypoints y ≤ Finset.card neighbors := by
  exact Finset.card_filter_le _ _

def points_at₀ (k: Fin 5) := Finset.card (
  Finset.filter (
    λ v ↦ mylist.1.get k + v ∈ mylist.1.take k) neighbors
)

#eval mylist.1.get (3:Fin 5)

#eval points_at₀ 4

def points_at {l:ℕ} (k: Fin l) (fold : Vector (ℤ×ℤ) l) (ph : Vector Bool l) :=
Finset.card ( -- under construction
  Finset.filter (λ v ↦
    ∃ i, i.1.succ < k.1 ∧
    ph.get k
    ∧
...
-- def pts_at {l:ℕ} (k : Fin l) (ph : Vector Bool l) (fold : Vector (ℤ×ℤ) l) : ℕ :=
--   ∑ i : Fin k, nat_of_bool (point_achieved fold ⟨i.1,finfin i⟩ k ph)
...
pts_at_with'' go₁ k ph fold := by
  ...
  simp
  simp at ha
  exact point_achieved_with_of_embeds h_embed fold ⟨a,finfin a⟩ k ph ha

def pts_tot' {α:Type}  {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α]
  {l:ℕ} (ph : Vector Bool l)(fold : Vector α l)
  := ∑ i : Fin l, pts_at_with' go i ph (fold)
...
-- theorem s (i : ℕ)  :
-- ∑ j in Finset.range (i+1), j = i.succ * i / 2
...
pts_tot' go₁ ph fold := by
  unfold pts_tot'; apply Finset.sum_le_sum; intros; exact pts_at_of_embeds' h_embed _ _ _ -- So much easier!
...
theorem Finset_sum_map_bound {α:Type}
[Fintype α]
{l:ℕ} (f : (Fin l) → α) {F₀ : α → ℕ} {c:ℕ}
  (h : ∀ a,    F₀ a     ≤     c) :
  ∑ i : Fin l, F₀ (f i) ≤ l * c  := by
    let Q := Finset.sum_le_card_nsmul Finset.univ (λ x ↦ F₀ (f x)) c (by tauto)
    simp at Q
    exact Q
...
theorem pts_achieved_bound'_ {α:Type}  [DecidableEq α]
{β : Type} [Fintype β]
(go : β → α → α)
{l:ℕ} (ph : Vector Bool l.succ) (fold : Vector α l.succ):
