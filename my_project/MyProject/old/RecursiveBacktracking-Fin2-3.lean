...
-- [Fintype α][DecidableEq α][DecidablePred fun a => P a ∨ Q a][DecidablePred fun x => P x ∧ Q x]
-- [DecidablePred fun x => Q x][DecidablePred P][Fintype {x : α // P x}][Fintype {x : α // Q x}]
...
--  by {
-- let sP : Finset α := Finset.filter (λ x ↦  P x) Finset.univ --Finset.subtype (λ a ↦ P a ∨ Q a) Finset.univ
-- -- have H : ∀ x : α, x ∈ s ↔ P x ∧ Q x := by {intro x;simp;}
-- have : Fintype.card { x : α // P x } = sP.card  := sorry
-- rw [this]

-- let sQ : Finset α := Finset.filter (λ x ↦  Q x) Finset.univ --Finset.subtype (λ a ↦ P a ∨ Q a) Finset.univ
-- -- have H : ∀ x : α, x ∈ s ↔ P x ∧ Q x := by {intro x;simp;}
-- have : Fintype.card { x : α // Q x } = sQ.card  := sorry
-- rw [this]

-- let s_or : Finset α := Finset.filter (λ x ↦  P x ∨ Q x) Finset.univ --Finset.subtype (λ a ↦ P a ∨ Q a) Finset.univ
-- have H : ∀ x : α, x ∈ s_or ↔ P x ∨ Q x := by {intro x;simp;}
-- have : Fintype.card { x : α // P x ∨ Q x } = s_or.card  := sorry
-- rw [this]

-- let s_and : Finset α := Finset.filter (λ x ↦  P x ∧ Q x) Finset.univ --Finset.subtype (λ a ↦ P a ∨ Q a) Finset.univ
-- -- have H : ∀ x : α, x ∈ s ↔ P x ∧ Q x := by {intro x;simp;}
-- have : Fintype.card { x : α // P x ∧ Q x } = s_and.card  := sorry
-- rw [this]

-- have maybe : Fintype.card { x : α // P x ∨ Q x } = (sP ∪ sQ).card := by {
--   sorry
-- }

-- have gfds : Fintype.card { x : α // P x ∨ Q x } =
-- Fintype.elems.card
-- := rfl

-- have gfds : Fintype.card { x : α // P x ∨ Q x } =
-- (Finset.univ : Finset { x : α // P x ∨ Q x }).card
-- := rfl

-- -- have sdghsdfg : (Finset.univ : Finset { x : α // P x ∨ Q x }) =
-- -- (Finset.univ : Finset { x : α // P x }) ∪ (Finset.univ : Finset { x : α // Q x })
-- -- := sorry

-- let Q := Finset.card_union_add_card_inter sP sQ
-- rw [← Q]
-- have : s_or = sP ∪ sQ := sorry
-- rw [this]

-- have : s_and = sP ∩ sQ := sorry
-- rw [this]


-- /-
-- EUREKA : just use
-- theorem Fintype.card_subtype_or_disjoint{α : Type u_1} (p : α → Prop) (q : α → Prop) (h : Disjoint p q) [Fintype { x : α // p x }] [Fintype { x : α // q x }] [Fintype { x : α // p x ∨ q x }] :
...
:= by {
  ...
  -- Fintype.card { v : Vector (Fin 2) 5 //  squarefree v ∧ g₀.1 <:+ v.1
   ...
   + Fintype.card { v : Vector (Fin 2) 5 // squarefree v ∧ g₁.1 <:+ v.1 }
:= by {
  let P := (λ v  : Vector (Fin 2) 5 ↦ squarefree v ∧ g₀.1 <:+ v.1 )
  let Q :=  (λ v  : Vector (Fin 2) 5 ↦ squarefree v ∧ g₁.1 <:+ v.1 )
  let hD : Disjoint P Q := by {
    unfold Disjoint
    intro S h0S h1S
    simp at h0S
    simp at h1S
    intro v hSv
    exfalso
    have h₀: squarefree v ∧ (capp 0 w h).1 <:+ v.1 := h0S v hSv
    have h₁: squarefree v ∧ (capp 1 w h).1 <:+ v.1 := h1S v hSv
    have H₀: (capp 0 w h).1 <:+ v.1 := h₀.2
    have H₁:  (capp 1 w h).1 <:+ v.1 := h₁.2
    rcases H₀ with ⟨t₀,ht₀⟩
    rcases H₁ with ⟨t₁,ht₁⟩
    rw [← ht₀] at ht₁
    unfold capp at ht₁
    simp at ht₁
    have : [1] ++ w.1 = 1 :: w.1 := by exact rfl
    have G₀: t₁ ++ [1] ++ w.1 = t₁ ++ 1 :: w.1:= by exact (List.append_cons t₁ 1 w.1).symm
    have G₁: t₀ ++ [0] ++ w.1 = t₀ ++ 0 :: w.1:= by exact (List.append_cons t₀ 0 w.1).symm

    have : t₁ ++ [1] ++ w.1 = t₀ ++ [0] ++ w.1:= Eq.trans G₀ (Eq.trans ht₁ G₁.symm)

    have : t₁ ++ [1] = t₀ ++ [0] := by exact (List.append_left_inj w.1).mp this
    have : (t₁ ++ [1]).getLast (by simp) = (t₀ ++ [0]).getLast (by simp) :=
      List.getLast_congr _ _ this
    simp at this
  }
  let G := Fintype.card_subtype_or_disjoint P Q hD
  -- Fintype.card { x : α // p x ∨ q x } = Fintype.card { x : α // p x } + Fintype.card { x : α // q x }
  rw [G]
}
