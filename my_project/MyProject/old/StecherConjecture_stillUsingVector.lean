...
(Finset.univ)) -- think "ik = (i,k)"

theorem finset_biunion_prod {α:Type}
[Fintype α]
[Fintype (α × α)]
[DecidableEq (α × α)]
 (P: α × α → Prop)
 [DecidablePred fun ik => P ik]
 [∀ k, DecidablePred fun ik : α×α => ik.2 = k ∧ P ik]
 :
 Finset.filter (fun ik : α × α => P ik) Finset.univ =
  Finset.biUnion Finset.univ (fun k : α =>
    Finset.filter (fun ik : α×α => ik.2 = k ∧ P ik) (Finset.univ : Finset (α×α))
  )
  := by
  apply Finset.ext
  intro ik
...
theorem finset_biunion_prod' {α β:Type} [Fintype α] [Fintype β] [DecidableEq (α × β)]
 (P: α × β → Prop)
 [DecidablePred fun ik => P ik]
 [∀ k, DecidablePred fun ik : α×β => ik.2 = k ∧ P (ik.1,k)]
 :
 Finset.filter (fun ik : α × β => P ik) Finset.univ =
  Finset.biUnion Finset.univ (fun k : β =>
    Finset.filter (fun ik : α×β => ik.2 = k ∧ P (ik.1,k)) (Finset.univ : Finset (α×β))
  )
  := by
  apply Finset.ext
  simp
...
theorem pts_tot'_eq_pts_tot'better
{α:Type} {β : Type} [Fintype β] (go : β → α → α)
[DecidableEq α] {l:ℕ} (ph : Vector Bool l) (fold : Vector α l)
:
pts_tot'better go ph fold = pts_tot' go ph fold := by
  unfold pts_tot' pts_tot'better pts_at'
  -- write pts_tot'better as a disjoint union over ik.2
  let α' := Fin l
  let β' := (Fin l) × (Fin l)
  ...
  have hDU: Finset.card U = ∑ k : Fin l, Finset.card (t k) := Finset.card_biUnion hD
  ...
  := by
    ...
    have h₂ : ∀ (a b : α') (ha : a ∈ s) (hb : b ∈ s), f a ha = f b hb → a = b
      := by
      intro ik₁ ik₂ hik₁ hik₂ hf
      simp at hf hik₁ hik₂
      exact Prod.ext hf (Eq.trans hik₁.1 hik₂.1.symm)
    have h₃ : ∀ b ∈ t, ∃ (a : α') (ha : a ∈ s), f a ha = b := by
      intro i hi;simp at hi;simp;tauto
