...
def wk_vec {α:Type} [OfNat α 0] {β : Type} [Fintype β] (go : β → α → α)
...
def walk_vec {α:Type} [OfNat α 0] {β : Type} [Fintype β] (go : β → α → α) (l : List β) : List α :=
  (wk_vec go l.reverse).1

def walk {α:Type} [OfNat α 0] {β : Type} [Fintype β] (go : β → α → α) (l : List β) : List α :=
  (wk go l.reverse).1
...
theorem moves_len {α β:Type} [OfNat α 0] [Fintype β] (go : β → α → α) (l : List β) :
  (walk go l).length = l.length.succ := by unfold walk; rw [moves_len']; simp
...
theorem walk_not_nil {b: ℕ} {α: Type} [OfNat α 0] (tail: List (Fin b)) (go: Fin b → α → α)
: walk go tail ≠ [] := by
    intro hc; apply congrArg List.length at hc; rw [moves_len go tail] at hc; simp at hc
...
= (go hd (anchor_vec (wk_vec go tl))) :: (wk_vec go tl).1 := rfl

theorem eq_anchor {b:ℕ} {α:Type} [OfNat α 0] (hd: Fin b) (tl: List (Fin b))
          (go: Fin b → α → α)  : wk go (hd :: tl)
= (go hd (anchor (wk go tl))) :: wk go tl := rfl
...
{go₀ : Fin b₀ → α → α} {go₁ : Fin b₁ → α → α} (h_embed : embeds_in go₀ go₁)
 {l:ℕ}
  ...
  simp; simp at htri; constructor; tauto; exact _is_' h_embed htri.2
...
theorem F_ {α:Type} [OfNat α 0] [DecidableEq α] {b₀ b₁ : ℕ}
...
theorem pts_embeds {α:Type} [OfNat α 0] [DecidableEq α] {b₀ b₁ : ℕ}
{go₀ : Fin b₀ → α → α} {go₁ : Fin b₁ → α → α} (h_embed : embeds_in go₀ go₁)
...
pts_tot go₁ ph fold := by
  ...
  cases hi; rw [← left,← right]; exact pts_at_embeds h_embed _ _ _
...
∀ fold : Vector α l, pts_tot go ph fold ≤ q
...
{l:ℕ} (ph : Vector Bool l) : P_ go₀ ph ≤ P_ go₁ ph := by
  have : ∀  q, P_bound go₁ ph q → P_bound go₀ ph q := by
    intro _ hq fold
    calc
    _ ≤ pts_tot go₁ ph fold := pts_embeds h_embed _ _
    _ ≤ _                          := hq fold
...
{l:ℕ} (ph : Vector Bool l.succ) : P_' go₀ ph ≤ P_' go₁ ph := by
  apply Nat.find_mono
  intro q hq moves
  let he := remove_strongly h_embed
  let Q := pts_embeds he ph (⟨(path go₀ moves.1).1,path_len _ _⟩)
...
theorem ptriquad' {l:ℕ} (ph : Vector Bool l.succ) : P_' tri ph ≤ P_' quad ph :=
p_embeds' embeds_strongly_tri_quad _

theorem pquadhex' {l:ℕ} (ph : Vector Bool l.succ) : P_' quad ph ≤ P_' move_hex' ph :=
p_embeds' embeds_strongly_quad_hex _
