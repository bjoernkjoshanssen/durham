...
theorem strengthen {a : PF 2}  {t₀:ℕ} {f:ℕ → ℕ}
  ...
  := by {
    ...
    exact Nat.mod_lt _ ((a.pos 0))
    -- rename_i n n_ih a_1 -- rename inaccessibles
    -- rename (∀ s ≤ n, f s ≠ PF.val a 0) → ∀ s ≤ n, f s < PF.val a 0 => n_ih
...
def CursiveNFA (a : PF 2) : NFA (Fin 1) (ℕ) := {
...
theorem generalh2s' (a : PF 2) (F: ℕ → ℕ)
(hw: curs_walks a F)
...
theorem getish' (F:ℕ → ℕ) (a : PF 2) (hw : curs_walks a F) :
...
: walk_' a (p 0) (a.val 0*(p 0) + 1 + a.val 1*(p 1)) = a.val 0 :=
  by {
    ...
    rw [add_assoc]
    have : PF.val a 0 * k + (1 + PF.val a 1 * l)
    = (PF.val a 0 * k + (PF.val a 1 * l)) + 1 := by ring
...
theorem keep_arriving (a : PF 2) (p : Fin 2 → ℕ)
: walk_' a (p 0) (Matrix.dotProduct a.val p).succ = a.val 0 :=
  by {
    rw [moveOne]
...
theorem unique_walk_cursive_helper' {a : PF 2} {w : ℕ → ℕ} {k : ℕ}
  ...
  by_cases h' : (w n_1=0); rw [if_pos h'] at g; simp at g; cases g
...
theorem afterwards {a : PF 2}{w: ℕ → ℕ}{k n: ℕ} (hnks: n ≥ (a.val 0 * k).succ)
  ...
  :
  w (Nat.succ n) = a.val 0 + (n - a.val 0 * k) % a.val 1
  := by {
...
theorem unique_walk_CursiveNFA {a : PF 2} {w : ℕ → ℕ} {k : ℕ}
  (hw: curs_walks a w)
  (hwk': w (a.val 0*k).succ = a.val 0 ∧ ∀ n < (a.val 0*k).succ, w n ≠ a.val 0) :
  w = walk_' a k :=
  by {
  ...
  apply funext; intro x; induction x; unfold curs_walks at hw; unfold walk_'
  ...
  rw [if_neg hn2k] at n_ih; unfold curs_walks at hw
...
def getk (i : CursiveWalk.Instance) (s : Solution_of i) : { k : ℕ // s.1 = walk_' i.cycleLength (k)}
:= by {
...
def getk1'' (i : CursiveWalk.Instance) (s : Solution_of i) : ℕ
  ...
  theorem getk2''
   (i : CursiveWalk.Instance) (s : Solution_of i)
  : s.1 = walk_' i.cycleLength (getk1'' i s)
  := by {
    let W := s; let w := W.1; let hw := W.2.1; let hu := W.2.2
...
def getk1  {u:ℕ} (a : PF 2) (W : { W : ℕ → ℕ // curs_walks a W ∧ W (Nat.succ u) = a.val 0}) : ℕ
  := by {
    let w := W.1
    let hw := W.2.1
    let hu := W.2.2
...
def getk1' (a : PF 2) {w : ℕ → ℕ} {u:ℕ}
(hw: curs_walks a w) (hu: w (Nat.succ u) = a.val 0) : ℕ
  := by {
    let t₀ := (find_spec_le (λ s ↦ w (Nat.succ s) = (a.val 0)) u hu).1
    let ht₀ := (find_spec_le (λ s ↦ w (Nat.succ s) = (a.val 0)) u hu).2
    let ish := getish' w a hw
  ...
  theorem getk2' (a : PF 2) {w : ℕ → ℕ} {u:ℕ}
  (hw: curs_walks a w) (hu: w (Nat.succ u) = a.val 0)
...
theorem l_unique (a : PF 2) (k l₁ l₂ : ℕ)
  (he: Matrix.dotProduct a.val (λ i ↦ [k,l₁].get i)
     = Matrix.dotProduct a.val (λ i ↦ [k,l₂].get i)) : l₁=l₂
  :=  by {
...
theorem getl' (a : PF 2) {k n:ℕ} (hmod₀: walk_' a k n = a.val 0)
...
theorem getl (a : PF 2) {k n:ℕ} (hmod₀: walk_' a k n = a.val 0)
:  {l : ℕ // n = Nat.succ (Matrix.dotProduct a.val (λ i ↦ [k,l].get i))}
:= by {
...
theorem walk_walks' (a : PF 2) (k:ℕ)
  : curs_walks a (walk_' a k) :=
  by {
...
theorem walk__injective' (a : PF 2) (k₁ k₂ : ℕ)
