...
example : ∃ x, x < 5 ∧ 1 ≠ 2 := by decide

example : ∃ x, x < 5 ∧ x ≠ 2 := by decide

-- example : padicValNat 2 4 = 2 := by decide

instance (a b:ℕ)  : Decidable (padicValNat 2 a = b) :=
by {
  exact Nat.decEq (padicValNat 2 a) b
}

instance (a b:ℕ) : Decidable (t a = b) :=
by {
  unfold t
  exact instDecidableEqNatPadicValNatOfNatInstOfNatNat a b
}
...
theorem bits_even {n:ℕ} (h : n ≠ 0) : Nat.bits (2*n) = false :: Nat.bits n := by {
  let G := bits_bool false h; rw [← G]; unfold Nat.bit; simp; unfold bit0; congr; ring
}
